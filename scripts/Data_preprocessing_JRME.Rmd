...
```{r}
data_cleaned_2 <- local({
  .Z <- scale(data_cleaned_1[,c("ME_Q2", "ME_Q3", "ME_Q4")])
  within(data_cleaned_1, {
    ME_Q4 <- .Z[,3]
    ME_Q3 <- .Z[,2]
    ME_Q2 <- .Z[,1]
  })
...
```{r}
RcmdrMisc::reliability(cov(data_cleaned_2[,c("ME_Q1","ME_Q2", "ME_Q3", "ME_Q4")], use ="complete.obs"))
```
...
data_cleaned_2$modelo_efectual <- 
  apply(data_cleaned_2[,c("ME_Q2", "ME_Q3", "ME_Q4")], 1, mean)

summary(data_cleaned_2$modelo_efectual)
...
```{r}
hist(data_cleaned_2$modelo_efectual)
```
...
```{r}
data_cleaned_3 <- local({
  .Z <- scale(data_cleaned_2[,c("NET_Q1", "NET_Q2", "NET_Q3", "NET_Q4")])
  within(data_cleaned_2, {
    NET_Q4 <- .Z[,4]
    NET_Q3 <- .Z[,3]
    NET_Q2 <- .Z[,2]
    NET_Q1 <- .Z[,1]
...
```{r}
RcmdrMisc::reliability(cov(data_cleaned_3[,c("NET_Q1", "NET_Q2", "NET_Q3", "NET_Q4")], use ="complete.obs"))
```
...
data_cleaned_3$networking <- apply(data_cleaned_3[,c("NET_Q1", "NET_Q2", "NET_Q3", "NET_Q4")], 1, mean)
summary(data_cleaned_3$networking)
...
```{r}
hist(data_cleaned_3$networking)
```
...
```{r}
cor(data_cleaned_3[,c("WOM_Q1", "WOM_Q2", "WOM_Q3")], use = "complete")
```

Ahora se procede a realizar la normalización de los items (Levin and Cross (MS2001)) y crear la variable final de networking. 

```{r}
data_cleaned_4 <- local({
  .Z <- scale(data_cleaned_3[,c("WOM_Q1", "WOM_Q2", "WOM_Q3")])
  within(data_cleaned_3, {
    WOM_Q3 <- .Z[,3]
    WOM_Q2 <- .Z[,2]
    WOM_Q1 <- .Z[,1]
  })
...
```{r}
RcmdrMisc::reliability(cov(data_cleaned_4[,c("WOM_Q1", "WOM_Q2", "WOM_Q3")], use ="complete.obs"))
```
...
data_cleaned_4$WOM <- apply(data_cleaned_4[,c("WOM_Q1", "WOM_Q2", "WOM_Q3")], 1, mean)
summary(data_cleaned_4$WOM)
...
```{r}
cor(data_cleaned_4[,c("DESEM_Q1", "DESEM_Q2", "DESEM_Q3", "DESEM_Q4")], use = "complete")
```

Ahora se procede a realizar la normalización de los items (Levin and Cross (MS2001)) y crear la variable final de networking. 

```{r}
data_cleaned_6 <- local({
  .Z <- scale(data_cleaned_4[,c("DESEM_Q1", "DESEM_Q2", "DESEM_Q3", "DESEM_Q4")])
  within(data_cleaned_4, {
    DESEM_Q4 <- .Z[,4]
    DESEM_Q3 <- .Z[,3]
    DESEM_Q2 <- .Z[,2]
    DESEM_Q1 <- .Z[,1]
...
```{r}
RcmdrMisc::reliability(cov(data_cleaned_6[,c("DESEM_Q1", "DESEM_Q2", "DESEM_Q3", "DESEM_Q4")], use ="complete.obs"))
```
...
data_cleaned_6$desempeno <- apply(data_cleaned_6[,c("DESEM_Q1", "DESEM_Q2", "DESEM_Q3", "DESEM_Q4")], 1, mean)
summary(data_cleaned_6$desempeno)
...
```{r}
data_tidied_1 <- data_cleaned_6 %>% inner_join(df_egonet)
```
