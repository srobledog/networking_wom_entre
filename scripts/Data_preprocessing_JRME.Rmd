...
Sin embargo, de acuerdo a los datos de cámara y comercio [leidy.aleja124\@gmail.com](mailto:leidy.aleja124@gmail.com){.email} solo tenía un registro y es una empresa que comercializa licores. Por lo tanto, se elimina la empresa del sector servicios que aparece a nombre de ella.
...
Por otro lado, [tatianavilla29\@outlook.es](mailto:tatianavilla29@outlook.es){.email} si tiene dos empresas registradas a nombre de ella. Una es una peluquería a nombre de VILLA PINEDA MARIA LUZ MARY y la otra una distribuidora a nombre de GARCES VILLA PAOLA TATIANA. Se eliminan los datos porque es difícil saber de quién es cada negocio. Más adelante se podría llamar
...
En esta parte desaparecieron 20 registros. Habían registros en la información de cámara y comercio que no habían en los datos registrados de la encuesta. Revisando un poco los datos se debe a respondieron con otro email diferente al que había en la base de datos. Por ejemplo: base de datos: [galgisa\@hotmail.com](mailto:galgisa@hotmail.com){.email} y encuesta: [calgisa\@gmail.com](mailto:calgisa@gmail.com){.email}. Esto hay que revisarlo después.
...
```{r}
data_cleaned_1 <- data_raw_5
```


### 1. Modelo Efectual: limpieza
...
```{r}
cor(data_cleaned_1[,c("ME_Q1", "ME_Q2", "ME_Q3", "ME_Q4")], use = "complete")
```

La pregunta 1 tiene una correlación por debajo de 0.4 con las otras variables, por lo tanto se elimina del constructo.

Ahora se procede a realizar la normalización de los items (Levin and Cross (MS2001)) y crear la variable final del Modelo efectual y networking.

```{r}
data_cleaned_2 <- 
  data_cleaned_1 |> 
  mutate(ME_Q1_sc = scale(ME_Q1),
         ME_Q2_sc = scale(ME_Q2),
         ME_Q3_sc = scale(ME_Q3),
         ME_Q4_sc = scale(ME_Q4),
         ME_Q1_tr = autoTransform(ME_Q1),
         ME_Q2_tr = autoTransform(ME_Q2),
         ME_Q3_tr = autoTransform(ME_Q3),
         ME_Q4_tr = autoTransform(ME_Q4),
         NET_Q1_sc = scale(NET_Q1),
         NET_Q2_sc = scale(NET_Q2),
         NET_Q3_sc = scale(NET_Q3),
         NET_Q4_sc = scale(NET_Q4),
         NET_Q1_tr = autoTransform(NET_Q1),
         NET_Q2_tr = autoTransform(NET_Q2),
         NET_Q3_tr = autoTransform(NET_Q3),
         NET_Q4_tr = autoTransform(NET_Q4)) |> 
  rowwise() |> 
  mutate(effectuation_mean = mean(c(ME_Q1_tr, 
                                    ME_Q2_tr, 
                                    ME_Q3_tr, 
                                    ME_Q4_tr)),
         networking_mean = mean(c(NET_Q1_tr, 
                                  NET_Q2_tr, 
                                  NET_Q3_tr, 
                                  NET_Q4_tr))) 
```

Como el pensamiento effectual es una variable formativa, no es necesario que se halle el Cronbach Alpha. 
...
rm(row, df_c1, df_c2, df_c3, df_c4, df_c5)
```

In this part, we'll remove the egonetwork. 

### 3. Actividades de networking: limpieza y organización.
...
```{r}
cor(data_cleaned_2[,c("NET_Q1_tr", "NET_Q2_tr", "NET_Q3_tr", "NET_Q4_tr")], 
    use = "complete")
```
...
```{r}
RcmdrMisc::reliability(cov(data_cleaned_2[,c("NET_Q1_tr", 
                                             "NET_Q2_tr", 
                                             "NET_Q3_tr", 
                                             "NET_Q4_tr")], 
                           use ="complete.obs"))
