...
```{r}
data_vivi_autscale_total_10_outliers <-  
  data_vivi_autscale_total_9_outliers 
```

Model

```{r}
Model_10 <- "# Measurement model
            Networking =~ NET_Q1 + NET_Q2 + NET_Q4
            WOM   =~ WOM_Q1 + WOM_Q2 + WOM_Q3
            Effectuation <~ ME_Q2 + ME_Q3 + ME_Q4
            # Regressions
            Networking ~ Effectuation
            WOM ~ Networking
            "
```
...
model_10_fit <- 
  lavaan::cfa(Model_10, 
              data = data_vivi_autscale_total_10_outliers)
```

Inspecting the factor loading for each item

```{r}
inspect(model_10_fit, what = "std")$lambda
```

Visualizing the results

```{r}
summary(model_10_fit, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
```

```{r}
fitMeasures(model_10_fit)
```


```{r}
semTools::reliability(model_10_fit)
```

```{r}
modificationindices(model_10_fit, sort. = TRUE)
```
...
```{r}
data_vivi_autscale_total_9_vero <- 
  data_raw_5 |> 
  left_join(df_egonet |> 
              dplyr::select(email, constraint, density, clustering)) |> 
  rowwise() |> 
...
data_vivi_center_networking_9_vero <-     # NETWORKING
  center_colmeans(data_vivi_autscale_total_9_vero |> 
                    select(NET_Q1_log, 
                           NET_Q2_log, 
                           NET_Q3_log , 
                           NET_Q4_log))  |> 
...
data_vivi_autscale_networking_9_vero <- 
  center_colstd.dev(data_vivi_center_networking_9_vero)

data_vivi_center_effectuation_9_vero <-   # EFFECTUTION
  center_colmeans(data_vivi_autscale_total_9_vero |> 
                    select(ME_Q1_log, 
                           ME_Q2_log, 
                           ME_Q3_log , 
...
data_vivi_autscale_effectuation_9_vero <- 
  center_colstd.dev(data_vivi_center_effectuation_9_vero) |> 
  rename("ME_Q1_trans" = ME_Q1_log,
         "ME_Q2_trans" = ME_Q2_log,
         "ME_Q3_trans" = ME_Q3_log,
         "ME_Q4_trans" = ME_Q4_log)

data_vivi_center_WOM_9_vero <-   # WOM
  center_colmeans(data_vivi_autscale_total_9_vero |> 
                    select(WOM_Q1_log, 
                           WOM_Q2_log, 
                           WOM_Q3_log )) 

data_vivi_autscale_WOM_9_vero <- 
  center_colstd.dev(data_vivi_center_WOM_9_vero) |> 
...
data_vivi_autscale_total_9_vero <- 
  data_vivi_autscale_total_9_vero |> 
  bind_cols(data_vivi_autscale_networking_9_vero,
            data_vivi_autscale_effectuation_9_vero,
            data_vivi_autscale_WOM_9_vero) |> 
  rowwise() |> 
...
data_vivi_autscale_total_9_vero_outliers <- 
  data_vivi_autscale_total_9_vero |>
  rowwise() |> 
  ...
  anomalize(constraint, method = "gesd") |> 
  dplyr::filter(anomaly != "Yes") |> 
  select(-constraint_l1, -constraint_l2, -anomaly) |> 
  anomalize(density, method = "gesd") |> 
  dplyr::filter(anomaly != "Yes") |> 
  select(-density_l1, -density_l2, -anomaly) |> 
  anomalize(clustering, method = "gesd") |> 
  dplyr::filter(anomaly != "Yes") |> 
  select(-clustering_l1, -clustering_l2, -anomaly)

datos_vero <- 
  data_vivi_autscale_total_9_vero_outliers |> 
  select(NET_Q1,NET_Q2,NET_Q3,NET_Q4,
         ME_Q1, ME_Q2, ME_Q3, ME_Q4,
         WOM_Q1, WOM_Q2, WOM_Q3,
         constraint, density, clustering
         )
```
...
```{r}
data_vivi_autscale_total_9_outliers_mod_effect <- 
  semTools::indProd(data_vivi_autscale_total_9_outliers,
...
model_9_fit <- 
  lavaan::cfa(Model_9, 
              data = data_vivi_autscale_total_9_outliers_mod_effect)
```

Inspecting the factor loading for each item

```{r}
inspect(model_9_fit, what = "std")$lambda
```

Visualizing the results

```{r}
summary(model_9_fit, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
```
